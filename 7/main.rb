...
protected
  ...
  def train_wagon_list
    ...
    @trains_list.each do|train|
      train.all_wagons {|wagon| puts "Вагон №: #{wagon.number_wagon}, тип вагона: #{wagon.type_wagon}, максимальная вместимость: #{wagon.capacity}, всего занято: #{wagon.busy}."} if train.number == t
    end
  end
  ...
  def station_train_list
    ...
    @stations_list.each do|station|
      station.all_trains {|train| puts "Поезд №: #{train.number}, тип поезда: #{train.type_train}, количество вагонов: #{train.wagons.size}."} if station.name == s
    end
  end
  ...
  def  filling_wagon
    ...
    w = gets.chomp.to_i
    @wagons_list.each do|wagon|
      if wagon.type_wagon == 'passenger'
        wagon.take_seat
      elsif wagon.type_wagon == 'cargo'
